...
/// A view that displays the ingredients and their measurements for the meal.
/// - parameter ingredients: A dictionary of ingredient names and their measurements.
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(ingredients.keys.sorted(), id: \.self) { key in
                if let measurement = ingredients[key], !measurement.isEmpty {
                    IngredientCardView(ingredient: key, measurement: measurement)
                }
            }
        }
        .padding()
